...
def generate_data(configs):
    ...
    theta_c = torch.randn(configs['dim_c'])
    theta_x = torch.randn(configs['dim_x'])
    theta_y = torch.randn(configs['dim_y'])
    ...
    A_c = torch.randn(configs['num_samples'], configs['dim_obs'], configs['dim_c'])
    A_x = torch.randn(configs['num_samples'], configs['dim_obs'], configs['dim_x'])
    B_c = torch.randn(configs['num_samples'], configs['dim_obs'], configs['dim_c'])
    B_y = torch.randn(configs['num_samples'], configs['dim_obs'], configs['dim_y'])
    ...
    data_x = torch.einsum('ndc,c->nd', A_c, theta_c_x) + torch.einsum('ndx,x->nd', A_x, theta_x) + noise_x
    # data_x = (theta_c_x @ A_c.T) + (theta_x @ A_x.T) + noise_x

    # Y = Bc * theta_c + By * theta_y + eps_Y
    # data_y = (theta_c @ B_c.T) + (theta_y @ B_y.T) + noise_y
    data_y = torch.einsum('ndc,c->nd', B_c, theta_c) + torch.einsum('ndy,y->nd', B_y, theta_y) + noise_y
