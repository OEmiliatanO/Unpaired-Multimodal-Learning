...
class Affectdataset(Dataset):
    ...
    def __init__(self, data: Dict, flatten_time_series: bool, aligned: bool = True, task: str = None, max_pad=False, max_pad_num=50, data_type='mosi', z_norm=False, vision_norm=False) -> None:
        ...
        # dataset mean for vision
        if vision_norm:
            # print("Normalizing vision data!!!!!!!!!!!!")
            key = 'vision'
            x = self.dataset[key].reshape(-1, self.dataset[key].shape[-1])
            mu = x.mean(0, keepdims=True)
            sigma = x.std(0, keepdims=True)
            self.dataset[key] = (self.dataset[key] - mu) / sigma
...
def get_dataloader(
        ...
        raw_path='/home/van/backup/pack/mosi/mosi.hdf5', z_norm=False, vision_norm=False) -> DataLoader:
    ...
    Args:
        ...
        vision_norm (bool, optional): Whether to normalize vision data or not. Defaults to False.
    Returns:
    ...
    if robust_test:
        ...
        for i in range(10):
            ...
            robust_text.append(
                DataLoader(Affectdataset(test, flatten_time_series, task=task, max_pad=max_pad, max_pad_num=max_seq_len, data_type=data_type, z_norm=z_norm, vision_norm=vision_norm), shuffle=False, num_workers=num_workers,
        ...
        # test = dict()
        test = DataLoader(Affectdataset(processed_dataset['test'], flatten_time_series, task=task, max_pad=max_pad, max_pad_num=max_seq_len, data_type=data_type, z_norm=z_norm, vision_norm=vision_norm), \
                      shuffle=False, num_workers=num_workers, batch_size=batch_size, \
                      collate_fn=process)
