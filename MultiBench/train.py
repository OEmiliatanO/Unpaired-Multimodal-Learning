...
def evaluate(model, config, ds_name='mosi'):
	...
	results[f'eval/modality_separate'] = np.array(modality_separate).mean()
	...
	for _iter in range(num_epoch):
		...
		for i_batch, (data_batch_1, data_batch_2) in enumerate(zip(train_loader_1, train_loader_2)):
			if ds_name != 'mimic':
				...
				x1_batch_lengths = data_batch_1[1].cuda() # shape: (B,)
				x2_batch_lengths = data_batch_2[2].cuda() # shape: (B,)
